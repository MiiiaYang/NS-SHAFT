...
void App::ValidTask() {
  ...
  //         if (m_Giraffe->GetImagePath() ==
  //         GA_RESOURCE_DIR"/Image/Character/giraffe.png") {
  ...
  //             LOG_DEBUG("The image path is {} instead.",
  //             m_Giraffe->GetImagePath());
  //         }
  //         break;
  ...
  //                 LOG_DEBUG("The giraffe collided with the chest but the
  //                 chest is still visible");
  //             } else {
  ...
  //             std::for_each(m_Doors.begin(), m_Doors.end(), [](const auto&
  //             door) { door->SetVisible(true); });

  //             m_PRM->NextPhase();
  //         } else {
  //             LOG_DEBUG("The bee animation is {} but not {}", isBeeLooping ?
  //             "looping" : "playing",
  ...
  //             LOG_DEBUG("Congratulations! You have completed Giraffe
  //             Adventure!"); m_CurrentState = State::END;
  //         } else{
